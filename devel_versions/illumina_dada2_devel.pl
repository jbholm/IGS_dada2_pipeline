...
if (!-e $r1fq || !-e $r4fq)
{
  ...
  {
    print LOG "Duration of R4 seqs.fastq production: $duration s\n";
...
## INPUT ##
# 1) provide Run mapping file (could be multiple projects)
# 2) provide Location of the raw GRC sample-specific .fq files (probably be .fq.gz)
# 3) provide variable region
# 5) provide run ID
# 6) provide full path to output final files (compressed sample specific files, .rds file, log file, )
## Ensure that everyone has permissions to write/cp to final output locations 
## rm filtered, tag cleaned fq, compress sample specific fastq files, then cp full project directory to provided output location
# Always run in scratch --- not in groupshare
# Possibly send email if error/complete -- wait in it. 
# PART2: Metadata incorporation would be put into part2 ---- Rscript for merging metadata to taxon data, merge, spit out list of sampleIDs w/o a match
# PART2: Jain's script for concatenating nSeqs after each step of dada2. 

####### Validate list file and provided runID ############ 
####### Check that the list file 2nd row contains the given runID #######
####### If not, exit w/warning #######
### Summarize the projects contained in the run
### Produce a run summary file via R script from bing
### Get notebook from Lauren for ITS sequences
### Cleanup script

####### COPY/DECOMPRESS SAMPLE-SPECIFIC GRC .gz FILES & RENAME ########
#######################################################################
    ...
    if ($var eq "ITS")
    {
      ...
      while ( $filename = readdir R1) 
      {
        if ($filename =~ /.fastq/)
        {
          ...
          my $Prefix = basename($filename, @suffixes);
          my $tc = "$wd/$Prefix"."_R1_tc";
          $cmd = "perl /usr/local/packages/tagcleaner-0.16/bin/tagcleaner.pl -fastq $r1seqs/$filename -out $tc -line_width 0 -verbose -tag5 CTGCCCTTTGTACACACCGC -mm5 2";
          print "\tcmd=$cmd\n" if $dryRun || $debug;
          system($cmd) == 0 or die "system($cmd) failed with exit code: $?" if !$dryRun;
          print LOG "$cmd\n";
        }
      ...
      while ( $filename = readdir R4) 
      {
        ...
        my $Prefix = basename($filename, @suffixes);
        my $tc = "$wd/$Prefix"."_R2_tc";
        $cmd = "/usr/local/packages/tagcleaner-0.16/bin/tagcleaner.pl -fastq $r4seqs/$filename -out $tc -line_width 0 -verbose -tag5 TTTCGCTGCGTTCTTCATCG -mm5 2";
        print "\tcmd=$cmd\n" if $dryRun || $debug;
        system($cmd) == 0 or die "system($cmd) failed with exit code: $?" if !$dryRun;
        print LOG "$cmd\n";
      }
...
if (scalar @removed > 0)
{
	print LOG scalar @removed . " samples removed during dada2 filtering:\n";
	for my $x (@removed)
	{
		print LOG "$x\n";
	}
...
if (-e $dadaTbl)
{
  ...
  print LOG "See $dadaTbl for dada2 table of reads surviving by step\n";
  close LOG;
}
else
{
...
open LOG, "<$log" or die "Cannot open $log for reading: $OS_ERROR";
while (<LOG>)
{
  ...
  if (defined $error)
  {
    ...
    while (<ERROR>)
    {
      if ($_ =~ /error/ || $_ =~ /Error/ || $_ =~ /ERROR/)
      {
        ...
        print LOG "Error in $step. See $error.\n";
        exit;
      }
      else
      {
        print "---No errors found in $step.\n";
        print LOG "---No errors found in $step.\n";
      }
    }
  }
  else
