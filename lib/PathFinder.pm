...
sub load_config_profile {
    ...
    if ( exists $self->{"config"}->{'part1 param profiles'}->{"$profile"} ) {
      ...
      # param profiles.
        my $profile
            = $self->{"config"}->{'part1 param profiles'}->{"$profile"};
        Hash::Merge::set_behavior('RIGHT_PRECEDENT');
        $self->{"config"}->{'part1 params'}
            = merge( $self->{"config"}->{'part1 params'}, $profile );
        return 1;
    }
    else {
        return 0;
...
sub override_default_param {
        ...
        && !defined($value) )
    {
        ...
        if ( defined($value) ) {
            if ( exists $self->{'config'}->{'part1 params'}->{$name} ) {
                $self->print(
                    "(Overriding default configured value for \"$name\" with value \"$value\" from CLI)\n"
                );
            }
            $ans = $value;
        }
            ...
            if ( !$optional ) {
                $self->print(
                    "Config has no default value for \"$name\" and value was not provided on the command line.\n"
                );
                die
                    "Config has no default value for \"$name\" and value was not provided on the command line.";
            }
...
sub part1_local_map {
    my $self = shift;
    return catdir( $self->{'wd'}, basename( $self->get_param('barcodes') ) );
}
...
sub splitsamples_output_fwd {
    ...
    my @globs = map { catfile( $self->sample_dir(), "*_R1.$_" ) } @extensions;
...
sub splitsamples_output_rev {
    ...
    my @globs = map { catfile( $self->sample_dir(), "*_R2.$_" ) } @extensions;
    my @result = glob( join " ", @globs );
    return @result;
}
