...
=head1 AUTHOR
 ...
 Forked from CPAN by Jonathan Lim on September 30, 2021
...
@EXPORT_OK = qw(user status brief_job_stats qacct all_jobs);
our $VERSION = '0.01';
    ...
    while (@status) {
        ...
        # 1441 0.56000 testing123 rob          r     03/25/2005 11:59:12     1
            ...
            while (@status) {
                ...
                my ( $user, $status, $date, $time, $processes )
                    = splice( @pieces, -5, 5 );
                my ( $pid, $load ) = splice( @pieces, 0, 2 );
                $date .= " " . $time;
                my $name = join " ", @pieces;
                $self->{'job'}->{$pid}
                    = [ 'pending', $pid, $load, $name, $user, $date ];
            }
        }
        else {
...
Get details on a job after it has completed.
Returns empty array if the job is still running.
